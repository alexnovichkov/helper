...
//Функция - это основной "кирпичик" программы, позволяющий избегать дублирования кода в программе.
...
//Для чего могут быть полезны функции:

//1. Организация кода. 
//Если писать весь код программы внутри функции main, то очень быстро можно запутаться,
//что делает тот или иной блок кода. Обычно программу можно разделить на некие логические
//куски (прочитай данные, выведи график, сохрани файл, посчитай значение), и эти куски
//удобно выносить в отдельные функции. Это позволяет организовать код функции main 
//таким образом, что ориенироваться в нем становится легче.

//2. Дублирование кода.
//Часто какой-то кусок кода требуется выполнить несколько раз. Например, если вы пишете
//консольную игру, то при каждом действии игрока нужно выводить изменившееся поле на экран.
//Код вывода поля, вынесенный в отдельную функцию, позволяет избежать дублирвования
//кода, если вывод поля требуется в разных частях программы.

//3. Тестирование
//Большую программу тестировать сложно. В большой программе всегда будут ошибки, так как
//написать код идеально не могут даже самые опытные программисты. Но если разбить код
//программы на отдельные логические блоки и вынести их в отдельные функции, то поиск ошибок
//упрощается: значительно проще написать функцию, которая делает что-то одно, и на разных
//входных данных всесторонне протестировать её работу, чем пытаться найти источник ошибки
//в длинном куске кода.

//Где размещать код функций в программе?
//--------------------------------------

//Компилятор C++ устроен таким образом, что при компиляции файла он движется строчка
//за строчкой, переводя код на C++ в машинные инструкции. При этом компилятор должен 
//знать, какие функции вызываются в каждой строке. Код ниже не скомпилируется:
...
//так как при компиляции функции main компилятору встретится вызов функции maximum,
//про которую компилятор ничего ещё не знает - она объявлена ниже.

//Один из способов решения - переместить функцию maximum в начало файла:
...
//К тому моменту, когда будет компилироваться функция main, компилятор уже будет знать,
//что такое maximum. Однако в больших программах бывает очень сложно определить, какая функция
//вызывает какую, и организовать их в нужном порядке. Иногда даже такого решения недостаточно:
//если функция A вызывает функцию B, а функция B вызывает функцию A, то любой порядок их 
//расположения будет вызывать ошибки компиляции.

//Поэтому в C++ принято делать так называемое предварительное объявление (forward declaration)
//функции:
//в начале файла пишут сигнатуры тех функций, которые будут определены ниже (сигнатуру
//функции main обычно опускают). 

//Что такое сигнатура? Это условно всё, что составляет функцию, но без тела функции 
//(без фигурных скобок). Сигнатуру функции заканчивают точкой с запятой:

int maximum(int first, int second); //сигнатура функции, которая будет определена ниже

int main()
{
    int a = 5, b = 4;
    std::cout << maximum(a, b); //компилятору достаточно сигнатуры функции выше.
}

int maximum(int first, int second) //повторяем сигнатуру и пишем тело (код) функции
{
    return first > second ? first : second;
}

//Предварительные объявления позволяют решить проблему циклического вызова функций:

void a(); //объявление функции, содержит только сигнатуру функции
void b();
void c();
...
void a() //определение функции, содержит сигнатуру и тело функции
{
    b();
}
...
//Программы из многих файлов
//==========================

//Простая программа будет содержать всего один файл cpp, в котором будет определение
//функции main и тех функций, которые вызываются в main.
//Однако если программа будет расти в размере, то количество функций будет также расти,
//и основной файл cpp превратится в длинную "лапшу", в которой будет трудно разобраться.
//Язык C++ позволяет разбивать текст программы на несколько файлов cpp, вынося в каждый
//кусок кода - обычно это одна или несколько функций.

//Однако тут проявляется особенность компилятора: компилируя один файл, он ничего не знает
//о других файлах программы - связыванием скомпилированных файлов занимается другая программа -
//линковщик. 

//Например, если в прокте есть два файла - main.cpp и functions.cpp с таким содержимым:
...
//то при компиляции файла main.cpp компилятор наткнется на имя maximum, про которое 
//он ничего не знает, и компиляция закончится с ошибкой.

//Тут на помощь приходит предварительное объявление функций:

//main.cpp
int maximum(int first, int second); //указание компилятору, что где-то ещё в программе определена такая функция
int main()
{
    int a = 5, b = 4;
    std::cout << maximum(a, b); //компилятору достаточно сигнатуры функции выше.
...
//Однако в больших программах значительно чаще объявления функций выносят в так называемый
