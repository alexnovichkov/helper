...
//Размер типа данных как раз и указывает, сколько ячеек (байт) в памяти будет отведено под
...
//Объявление переменной одновременно с присвоением значения называется инициализацией
//этой переменной. 
...
//5. nullptr - т.н. нулевой указатель.
...
//Такая переменная опасна, так как её значение может быть любым.
bool flag;
if (flag) {
   std::cout << "Destroying the universe in 3 sec...\n";
}

//Поэтому использовать неинициализированные переменные опасно, это может привести к 
//трудноуловимым ошибкам в работе программы. К счастью, современные редакторы кода
//умеют подсвечивать неинициализированные переменные и выводить соответствующие сообщения.


//Дополнительный материал. Рекомендуется читать после изучения темы structs_classes.h.
//Способы инициализации переменных
//--------------------------------

//1. Объявление неинициализированной переменной также называется инициализация
//по умолчанию. Для большинства простых типов данных это означает, что переменная
//вовсе не примет никакого значения (см. также выше).

double meaninglessVariable;
int anotherNonsence;

//Однако для таких типов данных, как классы и структуры (далее - просто класс), 
//если в классе есть конструктор по умолчанию, то он будет использован при инициализации
//объекта класса (инициализация по умолчанию с помощью конструктора). Пример:
...
IntHolder holder; //переменная инициализирована по умолчанию, так как был вызван
//соответствующий конструктор

//Кроме того, если в классе вовсе не объявлены конструкторы, то компилятор создаст
...
SimpleStruct ss; //переменная инициализирована по умолчанию.

//2. Инициализация присваиванием (copy assignment).

//Она отлично работает для простых типов данных:

int a = 45; //OK
bool condition = false; //OK
char ret = '\n';

//Но для классов такая инициализация не всегда удобна:

MyClass my1;
MyClass my2 = my1; //Будет работать, только если компилятор знает, как копировать 
//объекты класса MyClass

MyClass my3 = 4; //Будет работать, только если в классе MyClass объявлен имплицитный
//конструктор, принимающий один аргумент типа int.

//3. Прямая инициализация

//Или инициализация круглыми скобками. Означает, что мы напрямую задаем значение
//переменной при её объявлении. Отлично работает как для простых типов, так и для классов.

int y(9); //OK
char hyphen('-');
MyClass my4(4); //Если в классе есть конструктор, принимающий один аргумент типа int.

//У прямой инициализации есть один существенный недостаток: неявное преобразование
//типов.

int i(4.15); //Пытаемся инициализировать вещественным числом, дробная часть будет отброшена.
std::cout << i; //выведет 4

uint u(-4); //Warning! В переменной u окажется совсем не то, что ожидается.

//Кроме того, сложно отличить объявление функции от объявления переменной с прямой 
//инициализацией её значения:

MyClass my5(); //Объявление функции или переменной?

//4. Инициализация списком (фигурными скобками)
...
int g{}; //Инициализация списком по умолчанию. В большинстве компиляторов для int это значение 0.
int gg{3}; //Инициализация списком, содержащим один литерал.


//Инициализация списком отлично годится при объявлении переменных, имеющих тип структуры
//или класса с открытыми (public) полями:
...
Point p1{-3, 2.3}; //Инициализация списком

std::string myString{"some text"}; //Инициализация списком, содержащим символьный литерал.

//Инициализация списком также подходит для заполнения контейнера списком значений:

std::vector<double> myData{1.1, 2.2, 3.3, 4.4, 100};

//Инициализация списком не позволяет проводить неявное приведение типов:

int num{3.14}; //ошибка компиляции - неявное приведение типа, ведущее к потере точности,
//запрещено.
